...
abuse_reply = [
	...
	'Bhosadi ke aisa thappad marunga tere muh ke dant gad se hoker girenge.',	'Jija se bkchodi krta hai maderchod tamiz sikh lowde.',
...
def inbound_message():
    ...
    if str(number) in banned_users:
        ...
        print(f'<<< Bhosada Trap sent {send(number, "You are Banned kiddo. Contact *MrSp4rX* to be Unban.")}\n')

        send('919519874704', f"wa.me/{str(number)} is a Banned User and he/she wanna send {str(msg)} to me.")
        ...
        #     send('919519874704', f'wa.me/{str(number)} sent _*{str(msg)}*_ to *Bhosada Trap*')
            ...
            if str(number) == '919519874704':
                ...
                if 'ban' in str(msg):
                    ...
                    print(f"<<< Bhosada Trap sent {send('919519874704', temp)}")
                    ...
                    try:
                        ...
                        print(f"<<< Bhosada Trap sent {send('919519874704', 'Something Went Wrong...')}\n")
            ...
            send(number, 'No courses now')
                ...
                if len(msg)>=1:
                    ...
                    print(f'\n<<< Bhosada Trap sent {send(number, main(str(msg)))}\n')
            ...
            try:
                ...
                if times > 500:
                    ...
                    print(f"\n<<< Bhosada Trap Sent {send(number, '500 se jada nahi bhej skte')}\n")

                elif len(str(num)) > 10:

                    print(f"\n<<< Bhosada Trap Sent {send(number, 'Sahi Number likho')}\n")
                    ...
                    if str(num)=='9519874704':
                        ...
                        print(f"\n<<< Bhosada Trap Sent {send(number, 'Baap Se Backchodi nahi beta. Teko kya lagta hai *MrSparX* chutiya hai? Abhi dekhna teri kaise gand marta hai vo.')}\n")

                        print(f"\n<<< Bhosada Trap Sent {send('919519874704', f'wa.me/{number} wants to do SMS and Call Bombing on You. Be Careful Sir...')}\n")

                    else:

                        print(f"\n<<< Bhosada Trap Sent {send(number, 'Wow! Bombing Started...')}\n")

                        system(f'ispammer -m {str(times)} -t {str(num)}')

                        print(f"\n<<< Bhosada Trap Sent {send(number, 'Congratulations! Bombing Successfull...')}\n")

            except:

                print(f"\n<<< Bhosada Trap Sent {send(number, 'Something Went Wrong...')}\n")
